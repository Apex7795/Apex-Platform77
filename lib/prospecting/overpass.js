...
// Matching a city by name alone is genuinely ambiguous nationwide -- 30+
// "Springfield"s, multiple "Franklin"s, "Georgetown"s, "Salem"s, etc.
// Two-step area matching (find the state's boundary first, then find the
// city only within that boundary) resolves this properly. OSM boundary
// relations are named with the full state name, not the postal
// abbreviation tenants actually type ("CA"), so that needs mapping too.
...
function buildAreaClause(city) {
  const parts = city.split(',').map((s) => s.trim()).filter(Boolean);
  const cityName = escapeForRegex(parts[0] || '');
  const stateAbbr = (parts[1] || '').toUpperCase();
  const stateName = STATE_ABBREVIATIONS[stateAbbr];

  if (stateName) {
    // admin_level=4 is the US convention for state-level boundaries --
    // scoping the city lookup to inside that boundary is what actually
    // disambiguates same-named cities in different states.
    return `area["name"="${escapeForRegex(stateName)}"]["admin_level"="4"]->.stateArea;\narea["name"="${cityName}"](area.stateArea)->.searchArea;`;
  }
  // No recognizable state abbreviation -- best-effort fallback to the old
  // ambiguous name-only match rather than failing outright.
  return `area["name"="${cityName}"]->.searchArea;`;
}
