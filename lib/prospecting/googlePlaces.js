...
async function searchBusinesses({ query, city }) {
  ...
  if (!res.ok) {
    ...
    const error = new Error(`Google Places search failed: ${res.status} ${errBody}`);
    // Attach the real HTTP status so callers can tell "bad/invalid key"
    // (400/401/403) apart from a transient failure without string-parsing
    // the message -- a malformed/wrong-format key gets rejected as 400,
    // not 401, so both need to be treated as "not configured correctly."
