...
// (Numbers & senders > Facebook Messenger).
//
...
export async function POST(req) {
  ...
  try {
    ...
    // for this sender. A Facebook Page isn't mapped to a tenant_id anywhere
    // yet (there's no messenger equivalent of tracking_numbers), so a
    // Messenger contact that has never shown up as a lead through another
    ...
    // returns 0 rows once the app connects as the non-owner app_user role.
    const { rows } = await query(`SELECT get_tenant_for_caller_number($1) AS tenant_id`, [From]);
